...
SPEC_PATH="${1:-tests/e2e/admin-privacy-flows.spec.ts}"

if ! command -v docker >/dev/null 2>&1; then
  echo "Docker bulunamadı. Admin/Privacy E2E için Docker gereklidir."
  exit 1
fi

if ! docker info >/dev/null 2>&1; then
  echo "Docker daemon çalışmıyor. Docker Desktop veya daemon başlatılmalı."
  exit 1
fi
...
echo "Playwright admin/privacy spec çalıştırılıyor: ${SPEC_PATH}"
DATABASE_URL="${DB_URL}" \
REDIS_URL="${REDIS_URL:-redis://localhost:6379}" \
AUTH_COOKIE_SECURE=false \
corepack pnpm --filter @lexoffice/web e2e -- "${SPEC_PATH}"

...
function pickErrorMessage(body: unknown): string {
  ...
  if (!bodyObj) return 'RAG v3 integrity istegi basarisiz oldu.';
  ...
  return 'RAG v3 integrity istegi basarisiz oldu.';
}
...
export async function GET(request: Request) {
  try {
    ...
    const upstream = await fetchRagBackend('/api/v1/rag-v3/integrity', {
      method: 'GET',
      headers,

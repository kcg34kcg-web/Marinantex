...
const closeSchema = z.object({
  ...
  closure_code: z.string().min(2).max(60),
  reviewer_feedback: z.string().max(4000).optional(),
...
function pickErrorMessage(body: unknown): string {
  ...
  if (!bodyObj) return 'RAG v3 review close istegi basarisiz oldu.';
  ...
  return 'RAG v3 review close istegi basarisiz oldu.';
}
...
export async function POST(
  ...
  try {
    ...
    if (parsed.data.closed_by && parsed.data.closed_by !== userId) {
      return ragProxyErrorResponse({
        status: 403,
        errorCode: 'AUTH_SUBJECT_MISMATCH',
        message: 'closed_by alani oturum kullanicisi ile uyusmuyor.',
      });
    }
    const rateLimit = await enforceRagRouteRateLimit({
      request,
      routeKey: 'v3_review_close',
      userId,
    });
    ...
    const upstream = await fetchRagBackend(`/api/v1/rag-v3/review-queue/${encodeURIComponent(ticketId)}/close`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
        ...parsed.data,
        closed_by: parsed.data.closed_by ?? userId,
      }),
    ...
    const message = timedOut
      ? 'RAG v3 review close istegi zaman asimina ugradi.'
      : 'RAG v3 review close servisine baglanilamadi.';
    console.error('[RAG v3 review close proxy]', err, { backendCandidates: getRagBackendForLogs() });
    return ragProxyErrorResponse({
      status: 502,
